...
void s_meter_Disp()   {
  ...
  rssi = analogRead(AGC_PORT);

  s_value = map(rssi, 0, 4095, 0, 9);     // s max changed
...
// To display step frequency  
  ...
  if (fstep_idx == 1) display.print(F("  1Hz ")); 
  if (fstep_idx == 2) display.print(F(" 10Hz ")); 
  if (fstep_idx == 3) display.print(F("100Hz "));
  if (fstep_idx == 4) display.print(F("  1kHz")); 
  if (fstep_idx == 5) display.print(F(" 10kHz")); 
  if (fstep_idx == 6) display.print(F("100kHz"));
  if (fstep_idx == 7) display.print(F(" 1MHz "));
  if (fstep_idx == 8) display.print(F(" 25kHz"));
...
void rotary_event(uint8_t dir) {         // Rotary encoder Up/Down 
  ...
  if (mode == FREQ) {                    // FREQ:mode 0 : Up/Down frequency
    if (dir == 0x10){                    // Clockwise
      ...
      if (BandSelect == true) {          // AM mode limit
        if (freq1 > (long) 220000000) {  //  upper -> 220MHz
          freq1 = (long)   220000000;
        }
      }
      freq_0 = freq1;                // Written value for 5351a
      if (if_mode == true) {
        freq_0 = freq1  + interfreq  ;   // on IF  upper
      }
    ...
    if (dir == 0x20) {               // counter clockwise
      ...
      if(freq1<fstep) freq1 = freq1 + fstep; // If minus, return back
  ...
  else if (mode == STEP) {           // Up/Down step fstep_idx 1-8
    if (dir == 0x10) {               // Clockwise, increment  fstep_no
      fstep_idx ++;
      if (fstep_idx >= 9) fstep_idx = 1;
    }
    else if (dir == 0x20) {
      fstep_idx --;
      if (fstep_idx < 1) fstep_idx = 8;
    ...
    int cor_step = fstep;
    if (int(fstep)> 1000) cor_step = 1000;
    if (dir == 0x10) {               // Clockwise, increment  correction value
      freq_correction += cor_step;
      if (freq_correction >= 30000) freq_correction = 30000; // INT +32767 - 0 - -32767
    }
    else if (dir == 0x20) {
      freq_correction -= cor_step ;
      if (freq_correction <= -30000) freq_correction = -30000;
    }  
  }  
